...
void ePIC_DVCS_TASK::doAnalysis(){
  ...
  while(getline(fileListStream,fileName)){
	...
	Float_t t_recL = calcT_MethodL(beame4,beamp4,scate4_rec[0],fMass_proton,scatg4_rec[0]);
  ...
  for(int q{0}; q<nQ2bins; q++){
    for(int x{0}; x<nxBbins; x++){
      ...
      //h1DEff_t_Q2xB_Eff[q][x]->SetTotalHistogram(Form("h1_tMC_Q2xB_True[%i][%i]",q,x), "");
      //h1DEff_t_Q2xB_Eff[q][x]->SetPassedHistogram(Form("h1_tMCAcc_Q2xB_True[%i][%i]",q,x), "");
